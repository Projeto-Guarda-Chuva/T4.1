...
6. `GET /estado/presets` — listagem dos cinco estados predefinidos.
7. `POST /estado/preset?id=N` — aplicação do preset identificado por N (1 a 5).
8. `POST /estado/aleatorio` — aplicação de estado aleatório dentro dos limites válidos.
9. `GET /jornadas` — listagem resumida das jornadas em memória.
10. `GET /jornadas/detalhe?idx=N` — detalhamento de uma jornada específica com seus eventos.
11. `POST /jornadas/inicio` — abertura de uma nova jornada de movimentação.
12. `POST /jornadas/fim` — encerramento da jornada aberta.
13. `POST /emergencia` — acionamento manual da parada de segurança.
14. `POST /emergencia/reset` — reativação do sistema após emergência.
15. `GET /config` — leitura dos limites operacionais e da URL do registrador.
16. `POST /config/registrador` — atualização da URL de notificação externa.
17. `GET /logs` — leitura do buffer de logs em texto puro.
18. `DELETE /logs` — limpeza do buffer de logs.
